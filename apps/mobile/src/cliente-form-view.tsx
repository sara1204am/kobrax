/**
 * **El formulario de cliente. Uno solo, para toda la app.**
 *
 * Datos + teléfonos + ubicaciones + garantes (cada garante con sus propios teléfonos y ubicaciones).
 * Lo usan el alta (`cliente/nuevo`) y la edición (`cliente/editar`): si aparece un tercer lugar donde
 * se cargan datos de cliente, usa esta vista o no se hace. Antes vivía dentro de `cliente/nuevo.tsx`
 * y la edición tenía un formulario propio, más chico — por eso una dirección cargada mal no se podía
 * corregir.
 *
 * No sabe si crea o edita: recibe el estado y lo devuelve cambiado. Guardar es del que la usa.
 */
      ...
      <Accordion icon="👥" title="Garantes y contactos" badge={form.relations.length} defaultOpen>
        {form.relations.map((r) => (
          <ItemCard key={r.id} title={`👤 ${r.relatedName || 'Nueva persona'}`} onRemove={() => removeRelation(r.id)}>
            ...
            <LocationsSection locations={r.locations} setLocations={relLocations(r.id)} onError={onError} />
          </ItemCard>
        ))}
...
/** Lista de teléfonos (del cliente o de un garante). */
