...
/** La hoja de ruta en PDF. Mismo patrón que `api/uploads/[name]`: reenvía el archivo tal cual. */
export async function GET(_req: Request, { params }: { params: { id: string } }): Promise<Response> {
  let res: Response;
  try {
    res = await fetch(`${API_BASE}/routes/${params.id}/pdf`, { headers: bearerHeaders(), cache: 'no-store' });
  } catch {
