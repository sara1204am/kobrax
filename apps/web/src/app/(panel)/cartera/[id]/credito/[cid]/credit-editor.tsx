...
export function CreditEditor({
  ...
  const setExtra = (p: Partial<CreditDraft['extras']>) => onChange({ ...draft, extras: { ...draft.extras, ...p } });
  const setInitial = (p: Partial<CreditDraft['initial']>) => onChange({ ...draft, initial: { ...draft.initial, ...p } });
  ...
  return (
    <>
      {block === null ? (
        <>
          <section className="rounded-2xl border border-k-border bg-white p-5" aria-label={t('sections.terms')}>
            <h2 className="mb-4 text-[11px] font-semibold uppercase tracking-wide text-k-text-2">{t('sections.terms')}</h2>
            <CreditTermsFields form={draft.form} onChange={(form) => onChange({ ...draft, form })} />
          </section>
            ...
            <div className="grid gap-5 sm:grid-cols-3">
              ...
              <Field label={t('initial.outstandingBalance')}>
                <Input
                  ...
                  placeholder={registered?.ok && registered.balanceDerived ? t('initial.outstandingPlaceholder', { amount: money(registered.outstandingBalance, cur) }) : undefined}
            ...
            <p className="mt-4 text-[13px]" aria-live="polite">
              {registered === null ? (
                ...
                <span className="text-k-text">
                  {t('initial.result', { balance: money(registered.outstandingBalance, cur), date: dayDate(registered.nextDueDate, locale) })}
                </span>
              ) : (
                <span className="text-k-danger">{t(`initial.issues.${registered.code}`)}</span>
              )}
            </p>
            {derivedTotal == null && state.missing.length === 0 && <p className="mt-2 text-[12px] text-k-muted">{tc('quote.openLoan')}</p>}
          </Section>
        </>
      ) : (
      ...
      <Section title={t('sections.collection')}>
        <div className="grid gap-5 sm:grid-cols-2">
          <Field label={tp('form.status')}>
            <Select value={draft.extras.status} onChange={(e) => setExtra({ status: e.target.value })}>
          ...
          <Field label={tp('fields.code')}>
            <Input value={draft.extras.code} onChange={(e) => setExtra({ code: e.target.value })} maxLength={64} />
          </Field>

          {types.length > 0 && (
            <Field label={tp('fields.creditType')}>
              <Select value={draft.extras.typeCode} onChange={(e) => setExtra({ typeCode: e.target.value })}>
          ...
          {team.length > 0 && (
            <Field label={tp('form.assignedTo')}>
              <Select value={draft.extras.assignedManagerId} onChange={(e) => setExtra({ assignedManagerId: e.target.value })}>
          ...
          {(block === 'payments' || block === 'schedule') && (
            <Field label={t('nextDue')}>
              <Input type="date" value={draft.extras.nextDueDate} onChange={(e) => setExtra({ nextDueDate: e.target.value })} />
            </Field>
          )}
          ...
          <div className="sm:col-span-2">
            <Field label={t('notes')}>
              <Input
                ...
                onChange={(e) => onChange({ ...draft, form: { ...draft.form, notes: e.target.value } })}
