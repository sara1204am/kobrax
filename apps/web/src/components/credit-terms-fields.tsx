...
export function CreditTermsFields({
  ...
  form: CreditForm;
  onChange: (next: CreditForm) => void;
  /** «Lo cobra»: debajo de período · frecuencia · primer pago; en total acordado, al lado de la fecha. */
  assignee?: ReactNode;
  /** «Notas»: debajo; en total acordado, ocupando todo el ancho. */
  notes?: ReactNode;
}) {
  ...
  const rateHint =
    calculated &&
    form.ratePeriod !== RatePeriod.PER_INSTALLMENT &&
    form.ratePercent.trim() !== '' &&
    Number.isFinite(periodic)
      ? t('rateEquivalent', {
          rate: periodic.toLocaleString(locale, { maximumFractionDigits: 4 }),
          frequency: tp(`frequency.${form.frequency}`).toLowerCase(),
        })
      : undefined;
  const installments = termInstallments(form.installmentsCount, form.termUnit, form.frequency);
  const termHint =
    form.termUnit !== TermUnit.INSTALLMENTS &&
    form.installmentsCount.trim() !== '' &&
  ...
  const amortizations = [
    ...
    AmortizationMethod.FIXED_PRINCIPAL,
    ...(form.amortization === AmortizationMethod.SINGLE_PAYMENT
      ? [AmortizationMethod.SINGLE_PAYMENT]
      : []),
  ];
  ...
  const termUnits = calculated
    ? [
        TermUnit.YEARS,
        TermUnit.MONTHS,
        ...(form.termUnit === TermUnit.INSTALLMENTS ? [TermUnit.INSTALLMENTS] : []),
      ]
  ...
  const addCharge = () =>
    set({
      charges: [
        ...form.charges,
        { id: `n${Date.now()}`, label: '', kind: 'first_amount', value: '' },
      ],
    });
  ...
  const setAmortization = (amortization: AmortizationMethod) =>
    set({
      amortization,
      ...(amortization === AmortizationMethod.FIXED_PRINCIPAL
  ...
  return (
    <>
      <div className="mb-5 space-y-2">
        ...
        <div className="flex flex-wrap">
          <Segmented
            ...
            options={Object.values(CreditDefinition).map((d) => ({
              value: d,
              label: t(`definition.${d}`),
            }))}
          />
        </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        {/* Monto a prestar · Interés · Tipo de cuota, en una sola línea. */}
        <div className="grid gap-5 sm:col-span-2 sm:grid-cols-3">
          <Field label={t('principal')}>
            {numberInput(form.principal, (principal) => set({ principal }))}
          </Field>
            ...
            <Field label={t('ratePercent')} hint={rateHint}>
              ...
              <div className="flex gap-2">
                <div className="min-w-0 flex-1">
                  {numberInput(form.ratePercent, (ratePercent) => set({ ratePercent }))}
                </div>
                {ratePeriodApplies(form) && (
                  <div className="w-40">
                    <Select
                      aria-label={t('ratePeriod')}
          ...
          {agreedInstallment && (
            <Field label={t('installmentAmount')}>
              {numberInput(form.installmentAmount, (installmentAmount) =>
                set({ installmentAmount }),
              )}
            </Field>
          )}
          {agreedTotal && (
            <Field label={t('agreedTotal')}>
              {numberInput(form.agreedTotal, (agreedTotal) => set({ agreedTotal }))}
            </Field>
          )}
          {/* Forma de pago, al lado del total. Por defecto, pago único (`initialCreditForm`). */}
          {agreedTotal && (
            <Field label={t('repayment')}>
              <Select
                value={form.repayment}
                onChange={(e) => set({ repayment: e.target.value as RepaymentForm })}
              >
                {Object.values(RepaymentForm).map((r) => (
                  <option key={r} value={r}>
                    {t(`repaymentOptions.${r}`)}
                  </option>
                ))}
              </Select>
            </Field>
          ...
          {calculated && (
            <Field label={t('amortization')} hint={t(`amortizationHints.${form.amortization}`)}>
              {/* Por defecto, cuota fija (`initialCreditForm`). */}
              <Select
                value={form.amortization}
                onChange={(e) => setAmortization(e.target.value as AmortizationMethod)}
              >
                {amortizations.map((a) => (
        ...
        {/* Período del préstamo · Frecuencia de pago · Primer pago, en una sola línea. */}
        <div className="grid gap-5 sm:col-span-2 sm:grid-cols-3">
          {/* Número de cuotas: en el calculado de pago único es el plazo sobre el que corre el interés. */}
          {(calculated ||
            agreedInstallment ||
            (agreedTotal && form.repayment === RepaymentForm.INSTALLMENTS)) && (
            <Field
              label={calculated && single ? t('term') : t('installmentsCount')}
              hint={termHint}
            >
              ...
              <div className="flex gap-2">
                ...
                <div className="w-32">
                  <Select
                    aria-label={t('termUnit')}
                    value={form.termUnit}
                    onChange={(e) => set({ termUnit: e.target.value as TermUnit })}
                  >
                    {termUnits.map((u) => (
          ...
          {/* En total acordado la fila tiene lugar: «Lo cobra» va al lado de la fecha de pago. */}
          {agreedTotal && assignee}
        </div>

        {!agreedTotal && assignee}
        {/* Las notas: en total acordado, todo el ancho debajo de la fila. */}
        {notes && <div className={agreedTotal ? 'sm:col-span-2' : undefined}>{notes}</div>}
      </div>

      {/* Sólo lo que el dominio soporta hoy (D4, D8), más desgravamen y cargos (D18). */}
      {/* Siempre: el método de mora vale para cualquier definición. Lo demás, sólo en «Calcular cuotas». */}
      {
        <details className="group mt-5 rounded-xl border border-k-border">
          ...
          <div className="grid gap-5 border-t border-k-border p-4 sm:grid-cols-2">
            {/* D20: por defecto, el de la cuenta (lo pone quien arma el formulario). */}
            <Field label={t('arrearsMethod')} hint={t(`arrearsMethodHints.${form.arrearsMethod}`)}>
              <Select
                value={form.arrearsMethod}
                onChange={(e) => set({ arrearsMethod: e.target.value as ArrearsMethod })}
              >
              ...
              <>
                ...
                {form.amortization !== AmortizationMethod.FIXED_PRINCIPAL && (
                  <Field label={t('interestType')}>
                    <Select
                      ...
                      onChange={(e) => set({ interestType: e.target.value as InterestType })}
                    >
                ...
                {ratePeriodApplies(form) && form.ratePeriod !== RatePeriod.PER_INSTALLMENT && (
                  <Field label={t('rateConvention')}>
                    <Select
                      value={form.rateConvention}
                      onChange={(e) => set({ rateConvention: e.target.value as RateConvention })}
                    >
                ...
                {rateBaseApplies(form) && (
                  <Field label={t('rateBase')}>
                    <Select
                      value={form.rateBase}
                      onChange={(e) => set({ rateBase: e.target.value as InterestBase })}
                    >
                ...
                <Field label={t('insurance')} hint={t('insuranceHint')}>
                  {numberInput(
                    form.insuranceMonthlyPercent,
                    (insuranceMonthlyPercent) => set({ insuranceMonthlyPercent }),
                    '0.001',
                    '0',
                  )}
                </Field>
                ...
                <div className="space-y-3 sm:col-span-2">
                  ...
                  {form.charges.map((c) => (
                    <div
                      key={c.id}
                      className="grid gap-2 sm:grid-cols-[1fr_14rem_8rem_auto] sm:items-center"
                    >
                      <Input
                      ...
                      <Select
                        ...
                        value={c.kind}
                        onChange={(e) => setCharge(c.id, { kind: e.target.value as ChargeKind })}
                      >
                        {CHARGE_KINDS.map((k) => (
 ...
 */
export function CreditQuotePanel({
  state,
  currency,
  clientName,
  situation,
  ...
  currency: string;
  /** El cliente del crédito, arriba y en negrita; también nombra el CSV del plan. */
  clientName?: string;
  /** Cómo queda al registrarlo con cuotas ya pagadas (D13/D20): se muestra si hay pagadas o mora. */
  situation?: RegistrationSituation | null;
}) {
  const t = useTranslations('portfolio.creditForm');
  const locale = useLocale();
  const quote = state.calculation.quote;
  // El plan sólo se exporta completo: con datos faltantes o errores no hay plan que bajar.
  const schedule =
    state.missing.length === 0 && state.calculation.ok ? state.calculation.schedule : null;

  /** Descarga el plan de pagos en CSV, armado en el navegador (el crédito todavía no existe). */
  function exportPlan() {
    if (!schedule) return;
    downloadText(
      paymentPlanCsv(schedule),
      paymentPlanFileName(clientName, state.terms.firstDueDate),
    );
  }

  return (
    <section className="rounded-2xl border border-k-border bg-k-highlight p-5" aria-live="polite">
      {clientName && <p className="mb-4 text-[16px] font-bold text-k-navy">{clientName}</p>}
      {state.missing.length > 0 || !quote ? (
        ...
        <>
          <dl className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
            ...
            <Quote
              label={t('quote.total')}
              value={quote.total != null ? money(quote.total, currency) : '—'}
            />
            <Quote
              label={t('quote.profit')}
              value={quote.profit != null ? money(quote.profit, currency) : '—'}
            />
          </dl>
          {/* D13/D20: la situación al registrarlo — cuotas pagadas, saldo, próxima cuota y mora. */}
          {situation && (situation.paid > 0 || situation.daysPastDue > 0) && (
            <dl className="mt-4 space-y-1 border-t border-k-border pt-3 text-[13px]">
              <Extra
                label={t('situation.paidCount')}
                value={t('situation.paidOf', { count: situation.paid, total: situation.total })}
              />
              <Extra
                label={t('situation.outstanding')}
                value={money(situation.outstanding, currency)}
                strong
              />
              <Extra
                label={t('situation.next')}
                value={t('situation.nextValue', {
                  number: situation.next.number,
                  date: dayDate(situation.next.dueDate, locale),
                })}
              />
              <Extra
                label={t('situation.arrears')}
                value={
                  situation.daysPastDue > 0
                    ? `${t('situation.days', { count: situation.daysPastDue })} · ${t(
                        'situation.since',
                        {
                          date: dayDate(situation.arrearsSince, locale),
                        },
                      )}`
                    : t('situation.current')
                }
              />
              {situation.daysPastDue > 0 && (
                <p className="pt-1 text-[12px] text-k-danger">
                  {t('situation.oldestUnpaid', {
                    number: situation.next.number,
                    date: dayDate(situation.next.dueDate, locale),
                  })}
                </p>
          ...
          {quote.extras && (
            <dl className="mt-4 space-y-1 border-t border-k-border pt-3 text-[13px]">
              {quote.extras.insuranceTotal > 0 && (
                <Extra
                  label={t('quote.insurance')}
                  value={money(quote.extras.insuranceTotal, currency)}
                />
              )}
              {quote.extras.chargesTotal > 0 && (
                <Extra
                  label={t('quote.charges')}
                  value={money(quote.extras.chargesTotal, currency)}
                />
              )}
              {quote.extras.deducted > 0 && (
                <>
                  <Extra
                    label={t('quote.deducted')}
                    value={money(quote.extras.deducted, currency)}
                  />
                  <Extra
                    label={t('quote.netDisbursement')}
                    value={money(quote.extras.netDisbursement, currency)}
                    strong
                  />
                </>
              )}
          ...
          {quote.total == null && (
            <p className="mt-3 text-[13px] text-k-text-2">{t('quote.openLoan')}</p>
          )}
        </>
      )}
      {state.issues.map((i) => (
        <p
          key={i.code}
          className={`mt-3 text-[13px] ${i.severity === 'error' ? 'text-k-danger' : 'text-k-warning-text'}`}
        >
          {t(`issues.${i.code}`)}
        </p>
      ))}
      {schedule && schedule.length > 0 && (
        <div className="mt-5">
          <Button type="button" variant="ghost" onClick={exportPlan}>
            {t('exportPlan')}
          </Button>
        </div>
      )}
...
function Extra({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-3">
      ...
      <dd className={`tabular-nums ${strong ? 'font-semibold text-k-navy' : 'text-k-text'}`}>
        {value}
      </dd>
    </div>
  );
