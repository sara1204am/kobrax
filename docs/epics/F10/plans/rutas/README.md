...
- **⚠ Coordenadas de parada:** `route_stops` **NO** tiene lat/lng (sólo `clientId`, `sequenceOrder`, `status`, `caseId?`). `serializeStop` tampoco. Las coords salen de `GET /api/clients` → `locations[].{latitude,longitude}`. **Decisión:** el móvil hace el join `stop.clientId → client.location` en cliente (sin tocar backend). Techo: cargar clientes para resolver coords; aceptable porque los clientes ya se hidratan localmente (§4.1). *[ponytail: join en cliente; si pesa, extender serializeStop con la location primaria.]*
